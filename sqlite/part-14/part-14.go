...
func (r *Cursor) internalNodeInsert(parentPageNum, childPageNum int) {
	...
	if originalNumKeys >= INTERNAL_NODE_MAX_CELLS {
		// TODO: internal_node full
		// 1.拆分
		// 2.插入
		// 3.更新
		r.internalNodeSplitAndInsert(parentPageNum, childPageNum)
		return
	}
...
func (r *Cursor) internalNodeSplitAndInsert(oldPageNum, childPageNum int) {
	// B+树的分裂只影响原结点和父结点，而不会影响兄弟结点

	// old
	oldInternalPage := r.table.pager.getPage(oldPageNum)
	oldInternalNode := NewNode(&oldInternalPage)

	// new
	newInternalPageNum := r.table.pager.getUnusedPageNum()
	newInternalPage := r.table.pager.getPage(newInternalPageNum)
	newInternalNode := NewNode(&newInternalPage)
	newInternalNode.initializeInternalNode()

	newInternalNode.setNodeParent(oldInternalNode.nodeParent())

	// child
	childPage := r.table.pager.getPage(childPageNum)
	childNode := NewNode(&childPage)
	childMaxKey := childNode.getNodeMaxKey()
	index := oldInternalNode.internalNodeFindChild(childMaxKey)

	// 1.拆分
	for i := INTERNAL_NODE_MAX_CELLS; i >= 0; i-- {
		var destinationNode *Node
		if i >= INTERNAL_NODE_LEFT_SPLIT_COUNT {
			// new (right)
			destinationNode = newInternalNode
		} else {
			// old (left)
			destinationNode = oldInternalNode
		}

		// 2.插入
		// 新页 cell索引位置
		indexWithinNode := i % INTERNAL_NODE_LEFT_SPLIT_COUNT
		if i == index {
			destinationNode.setInternalNodeChild(0, IntToBytes(childPageNum))
			destinationNode.setInternalNodeKey(0, childMaxKey)
		} else if i > index {
			// 往后移一位
			destinationNode.setInternalNodeCell(indexWithinNode, oldInternalNode.internalNodeCell(i-1))
		} else {
			destinationNode.setInternalNodeCell(indexWithinNode, oldInternalNode.internalNodeCell(i))
		}
	}

	// 3.更新
	// 更新cell数量
	oldInternalNode.setInternalNodeNumKeys(INTERNAL_NODE_LEFT_SPLIT_COUNT)
	newInternalNode.setInternalNodeNumKeys(INTERNAL_NODE_RIGHT_SPLIT_COUNT)
	// 更新最右指针
	newInternalNode.setInternalNodeRightChild(oldInternalNode.internalNodeRightChild())
	newLeafPageNum := r.table.pager.getUnusedPageNum()
	oldInternalNode.setInternalNodeRightChild(IntToBytes(newLeafPageNum))

	r.table.pager.pages[oldPageNum] = oldInternalNode.data
	r.table.pager.pages[newLeafPageNum] = newInternalNode.data

	if oldInternalNode.isNodeRoot() {
		// 创建rootNode
		r.table.createNewRoot(newInternalPageNum)
	} else {
		// 非root内部节点
		parentPageNum := oldInternalNode.nodeParent()
		newMax := oldInternalNode.getNodeMaxKey()
		parentPage := r.table.pager.getPage(int(parentPageNum))
		parent := NewNode(&parentPage)

		oldMax := oldInternalNode.getNodeMaxKey()
		parent.updateInternalNodeKey(oldMax, newMax)
		r.internalNodeInsert(int(parentPageNum), newInternalPageNum)
	}
