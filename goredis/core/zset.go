...
// create zset
func createZsetObject() *GoredisObject {
	val := new(zSet)
	val.dict = new(dict)
	dict := make(map[string]*GoredisObject)
	*val.dict = dict
...
func zSetAdd(zObj *GoredisObject, score float64, ele string, flags *int, newScore *float64) bool {
...
func dictFind(d *dict, key string) *GoredisObject {
	if (*d)[key] != nil {
		return (*d)[key]
	}
	return nil
...
func zsetScore(zobj *GoredisObject, member string, score *float64) int {
	if zobj == nil || member == "" {
		return C_ERR
	}
