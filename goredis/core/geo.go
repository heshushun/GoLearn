	...
	for i := 0; i < elements; i++ {
		...
		if lngObj, ok1 := c.Argv[i*3+2].Ptr.(string); ok1 {
			if latObj, ok2 := c.Argv[i*3+3].Ptr.(string); ok2 {
				...
				if ok != nil {
					c.addReplyError("lng lat type error")
					os.Exit(0)
				}
			}
		}
...
func GeoDistCommand(c *Client, s *Server) {
	...
	c.addReplyError(fmt.Sprint(buf))
}
...
func georadiusGeneric(c *Client, flags uint) {
	...
	if err != nil {
		c.addReplyError("radius_meters error")
		return
	}
...
//单位
func extractUnitOrReply(c *Client, uint GoredisObject) float64 {
	u := uint.Ptr.(string)
...
func membersOfAllNeighbors(zobj *GoredisObject, n GeoHashRadius, lon float64, lat float64, radius float64, ga *geoArray) int {
...
func membersOfGeoHashBox(zobj *GoredisObject, hash GeoHashBits, ga *geoArray, lon float64, lat float64, radius float64) int {
...
func geoGetPointsInRange(zobj *GoredisObject, min float64, max float64, lon float64, lat float64, radius float64, ga *geoArray) int {
